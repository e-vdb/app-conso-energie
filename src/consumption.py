...
class ConsumptionDataframe:
    def __init__(self, df):
        ...
        self.saved_columns = [col for col in self.df.columns if col not in zero_cols]
    ...
    def evaluate_consumption(self):
        ...
        self.df['Month_consumption'] = self.df['date'].apply(lambda x: x - relativedelta(months=1)).dt.month_name()
        self.df['Year_consumption'] = self.df['date'].apply(lambda x: x - relativedelta(months=1)).dt.year
    ...
    def filter_year(self, year):
        ...
        filt = (self.df['date'] >= year + '-02-01') & (self.df['date'] < f'{str(int(year) + 1)}-02-01')
